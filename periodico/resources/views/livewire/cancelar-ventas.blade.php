                                ...
                                @foreach ($ventas as $result)
                                    <tr>
                                        ...
                                        <td class="px-4 py-2 border border-dark">
                                            ...
                                            @else
                                                <button class="px-2 py-1 cursor-pointer bg-green-500 hover:bg-green-600 text-white my-2 rounded-lg">
                                                    {{ $tipo === 'suscripciones' ? 'Suscripción cancelada' : 'Venta cancelada' }}
                                                </button>
                                                <button wire:click="verPDF('{{ $tipo == 'ventas' ? $result->idVenta : $result->idSuscripcion  }}')" class="px-2 py-1 cursor-pointer bg-blue-500 hover:bg-blue-600 text-white my-2 rounded-lg">
                                                    {{ 'Ver PDF' }}
                                                </button>
                                            @endif
                                        </td>
